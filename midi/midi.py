...
"""
This script generates MIDI files from a given input.
It uses the `midiutil` library.
It can generate a MIDI which contains every single note on a standard piano.
"""
...
def generate_midi(filename: str, tempo: int = 120) -> None:
    ...
    for note in range(12, 128):
        midi.addNote(track=track, channel=channel, pitch=note, time=note - 12, duration=duration, volume=volume)  # Track 0, Channel 0, Note, Start Time, Duration, Velocity
    ...
    for i, chord in enumerate(chords):
        for note in chord:
            midi.addNote(track=track, channel=channel, pitch=note + 48, time=128 + i * 1, duration=duration, volume=volume)  # Track 0, Channel 0, Note, Start Time, Duration, Velocity
